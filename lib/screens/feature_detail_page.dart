...
class _FeatureDetailPageState extends State<_FeatureDetailPage> {
  ...
  final List<_ChatMessage> _messages = [
    ...
    _ChatMessage(author: 'Berny', text: 'Necesito ayuda con un pago pendiente.'),
  ];
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(widget.feature.title, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF173726))),
                    ),
                  ],
                ),
  ...
  List<Widget> _buildSections(BuildContext context) {
    switch (widget.feature) {
      ...
      case QuickFeature.services:
        return [
          _ServicesInteractiveSection(
            ...
            onPay: () => _mockAction(() => _showSnackBar('Servicios pagados con éxito')),
          ),
        ];
      ...
      case QuickFeature.accountStatement:
        return [
          _StatementInteractiveSection(
            ...
            onRangeChanged: (index) => setState(() => _statementRangeIndex = index),
            exported: _showStatementExport,
            onExport: () => _mockAction(() => setState(() => _showStatementExport = true)),
          ),
        ];
      ...
      case QuickFeature.schedule:
        return [
          _ScheduleInteractiveSection(
            ...
            onToggleReminder: (value) => setState(() => _weeklyReminder = value),
          ),
        ];
      ...
      case QuickFeature.academicScore:
        return [
          _ScoreInteractiveSection(
            ...
            onProgressChanged: (value) => setState(() => _scoreProgress = value),
          ),
        ];
      ...
      case QuickFeature.academicCalendar:
        return [
          _CalendarInteractiveSection(
            ...
            onFilterChanged: (index) => setState(() => _calendarFilterIndex = index),
          ),
        ];
      ...
      case QuickFeature.support:
        return [_SupportInteractiveSection(controller: _chatController, messages: _messages, onSend: _sendMessage)];
    }
  }
...
class _TuitionSection extends StatelessWidget {
  const _TuitionSection({required this.paid, required this.selectedMethods, required this.onToggleMethod, required this.onPay});
...
class _CreditsInteractiveSection extends StatelessWidget {
  const _CreditsInteractiveSection({required this.selectedCredits, required this.onChanged});
  ...
  Widget build(BuildContext context) {
    ...
    return _GlassCard(
      child: Column(
        ...
        children: [
          const Text('Pago de créditos', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          Text('Créditos inscritos: $selectedCredits', style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF173726))),
          Slider(
            min: 0,
            max: 20,
            divisions: 20,
          ...
          const _SummaryLine(label: 'Estado del pago', value: 'Pendiente parcial'),
...
class _ServicesInteractiveSection extends StatelessWidget {
  const _ServicesInteractiveSection({required this.selectedServices, required this.onToggle, required this.onPay});
  ...
  @override
  Widget build(BuildContext context) {
    final options = ['Laboratorio', 'Biblioteca', 'Carnet', 'Graduación', 'Constancias', 'Certificaciones', 'Reposición de carnet'];
    final total = selectedServices.length * 350;
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Pago de servicios', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: options.map((service) => FilterChip(
              label: Text(service),
          ...
          const SizedBox(height: 12),
          Text('Servicios seleccionados: ${selectedServices.length}', style: const TextStyle(fontWeight: FontWeight.w700)),
...
class _BalanceInteractiveSection extends StatelessWidget {
  const _BalanceInteractiveSection({required this.showDebtView, required this.onToggleView});
    ...
    return _GlassCard(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(showDebtView ? 'RD\$ 12,450' : 'RD\$ 0.00', style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: showDebtView ? const Color(0xFFC03A2B) : const Color(0xFF0E5A38))),
          const SizedBox(height: 10),
          SimpleLineChart(values: showDebtView ? const [0.7, 0.68, 0.72, 0.74, 0.8, 0.88, 0.92] : const [0.2, 0.22, 0.19, 0.3, 0.28, 0.24, 0.12]),
          const SizedBox(height: 10),
          Text(showDebtView ? 'Estado con deuda pendiente.' : 'Cuenta al día y sin balance pendiente.', style: const TextStyle(fontSize: 12, color: Color(0xFF62716A))),
        ],
      ),
    );
  }
...
class _StatementInteractiveSection extends StatelessWidget {
  const _StatementInteractiveSection({required this.rangeIndex, required this.onRangeChanged, required this.exported, required this.onExport});
  ...
  Widget build(BuildContext context) {
    ...
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Estado de cuenta', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8,
            runSpacing: 8,
          ...
          if (exported) ...[
            const SizedBox(height: 12),
            const _StatusPill(label: 'PDF exportado', color: Color(0xFF1B7A4B), background: Color(0xFFE7F5EC)),
          ],
          const SizedBox(height: 12),
          FilledButton.icon(onPressed: onExport, icon: const Icon(Icons.picture_as_pdf_outlined), label: const Text('Exportar PDF')),
        ],
      ),
    );
  }
...
class _SubjectSelectionSection extends StatelessWidget {
  const _SubjectSelectionSection({required this.availableSubjects, required this.selectedSubjects, required this.onToggle});
  ...
  @override
  Widget build(BuildContext context) {
    final totalCredits = availableSubjects.where((subject) => selectedSubjects.contains(subject.name)).fold<int>(0, (sum, subject) => sum + subject.credits);
    ...
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Selección de materias', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 8),
          Text('Total de créditos seleccionados: $totalCredits', style: const TextStyle(fontWeight: FontWeight.w700)),
          const SizedBox(height: 8),
          if (hasConflict)
            const _StatusPill(label: 'Validación: posible choque de horario', color: Color(0xFFC03A2B), background: Color(0xFFFCEAE8))
          else
            const _StatusPill(label: 'Validación: sin conflictos', color: Color(0xFF1B7A4B), background: Color(0xFFE7F5EC)),
          const SizedBox(height: 12),
          ...
          const SizedBox(height: 8),
          _ActionPill(label: 'Inscribir materias', icon: Icons.playlist_add_check_outlined),
        ],
      ),
    );
  }
...
class _SelectableSubjectCard extends StatelessWidget {
  const _SelectableSubjectCard({required this.subject, required this.selected, required this.onToggle});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: selected ? const Color(0xFF1B7A4B) : const Color(0xFFE2ECE4)),
      ),
      child: Row(
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(subject.name, style: const TextStyle(fontWeight: FontWeight.w800, color: Color(0xFF173726))),
                const SizedBox(height: 4),
                Text('${subject.credits} créditos · ${subject.time} · ${subject.room}', style: const TextStyle(fontSize: 12, color: Color(0xFF62716A))),
              ],
            ),
          ),
          FilledButton.tonal(onPressed: onToggle, child: Text(selected ? 'Quitar' : 'Agregar')),
        ],
      ),
    );
  }
...
class _ScheduleInteractiveSection extends StatelessWidget {
  const _ScheduleInteractiveSection({required this.reminderEnabled, required this.onToggleReminder});
    ...
    return _GlassCard(
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Expanded(child: Text('Horario semanal', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726)))),
...
class _RecordInteractiveSection extends StatelessWidget {
  const _RecordInteractiveSection({required this.controller, required this.onSearch});
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Record académico', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          TextField(
...
class _ScoreInteractiveSection extends StatelessWidget {
  const _ScoreInteractiveSection({required this.progress, required this.onProgressChanged});
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Score académico', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          SimpleCircularProgress(value: progress, label: 'Índice General', subtitle: '3.72 / 4.00'),
          const SizedBox(height: 12),
          Slider(value: progress, min: 0.0, max: 1.0, onChanged: onProgressChanged),
...
class _NewsInteractiveSection extends StatelessWidget {
  const _NewsInteractiveSection({required this.favoriteNews, required this.onToggleFavorite});
  ...
  Widget build(BuildContext context) {
    const news = [
      ('Nueva convocatoria de becas', 'Abierta la solicitud para estudiantes de alto rendimiento con balance al día.', 'Académico', 'Hoy', Color(0xFF0E5A38)),
      ('Calendario de exámenes publicado', 'Consulta fechas por cuatrimestre y recordatorios del próximo período.', 'Eventos', 'Ayer', Color(0xFF4D6B2F)),
    ];
                ...
                child: Row(
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(18),
                        gradient: LinearGradient(colors: [item.$5.withValues(alpha: 0.9), item.$5.withValues(alpha: 0.55)]),
                      ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              _StatusPill(label: item.$3, color: item.$5, background: item.$5.withValues(alpha: 0.12)),
                              const Spacer(),
                              IconButton(
                                onPressed: () => onToggleFavorite(item.$1),
                                icon: Icon(favoriteNews.contains(item.$1) ? Icons.favorite : Icons.favorite_border, color: Colors.redAccent),
                              ),
                            ],
                          ),
                          Text(item.$1, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
                          const SizedBox(height: 6),
                          Text(item.$2, style: const TextStyle(fontSize: 13, color: Color(0xFF62716A), height: 1.3)),
                          const SizedBox(height: 6),
                          Text(item.$4, style: const TextStyle(fontSize: 12, color: Color(0xFF62716A))),
                        ],
                      ),
                    ),
                  ],
...
class _CalendarInteractiveSection extends StatelessWidget {
  const _CalendarInteractiveSection({required this.filterIndex, required this.onFilterChanged});
  ...
  Widget build(BuildContext context) {
    ...
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Calendario académico', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: List.generate(filters.length, (index) => ChoiceChip(
          ...
          const SizedBox(height: 12),
          Text('Filtro actual: $selectedFilter', style: const TextStyle(fontWeight: FontWeight.w700)),
          const SizedBox(height: 12),
          const _TimelineItem(title: 'Inscripción', subtitle: '1 - 5 de agosto', color: Color(0xFF0E5A38)),
          const _TimelineItem(title: 'Retiros', subtitle: '10 - 12 de agosto', color: Color(0xFF4D6B2F)),
          const _TimelineItem(title: 'Exámenes finales', subtitle: '28 - 31 de agosto', color: Color(0xFF164D64)),
        ],
      ),
    );
  }
...
class _LibraryInteractiveSection extends StatelessWidget {
  const _LibraryInteractiveSection({required this.controller, required this.autoSearch, required this.onToggleAutoSearch, required this.onSearch});
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Biblioteca Virtual', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          TextField(
            controller: controller,
            decoration: const InputDecoration(hintText: 'Buscar libros', prefixIcon: Icon(Icons.search)),
            onSubmitted: (_) => onSearch(),
          ),
          const SizedBox(height: 12),
          SwitchListTile(
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Campus Virtual', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              FilledButton.tonal(onPressed: () => onLaunch('Abriendo Google Classroom'), child: const Text('Google Classroom')),
              FilledButton.tonal(onPressed: () => onLaunch('Abriendo Moodle'), child: const Text('Moodle')),
              FilledButton.tonal(onPressed: () => onLaunch('Abriendo Teams'), child: const Text('Teams')),
              FilledButton.tonal(onPressed: () => onLaunch('Abriendo Zoom'), child: const Text('Zoom')),
            ],
          ),
        ],
      ),
...
class _SupportInteractiveSection extends StatelessWidget {
  const _SupportInteractiveSection({required this.controller, required this.messages, required this.onSend});
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Chat de soporte', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF173726))),
          const SizedBox(height: 12),
          ...messages.map((message) => Padding(
                padding: const EdgeInsets.only(bottom: 10),
                child: Align(
                  alignment: message.author == 'Berny' ? Alignment.centerRight : Alignment.centerLeft,
                  child: Container(
                    constraints: const BoxConstraints(maxWidth: 320),
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
                      color: message.author == 'Berny' ? const Color(0xFFE7F5EC) : const Color(0xFFF8FBF8),
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(message.author, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Color(0xFF62716A))),
                        const SizedBox(height: 4),
                        Text(message.text, style: const TextStyle(color: Color(0xFF173726))),
                      ],
                    ),
                  ),
                ),
          ...
          TextField(
            controller: controller,
            decoration: const InputDecoration(hintText: 'Escribe tu mensaje', prefixIcon: Icon(Icons.chat_bubble_outline)),
            onSubmitted: (_) => onSend(),
          ),
          const SizedBox(height: 12),
          FilledButton.icon(onPressed: onSend, icon: const Icon(Icons.send_outlined), label: const Text('Enviar')),
        ],
      ),
    );
  }
